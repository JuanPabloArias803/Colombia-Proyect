...
export async function DataView(){
    ...
    let dataArray:city[]=await transformCsvData((CryptoJS.AES.decrypt(fileContent, 'key-here').toString(CryptoJS.enc.Utf8)));
   ...
   `;
    ...
    const $download=document.querySelector(".download-link") as HTMLAnchorElement;
    ...
    function downloadLink():void {       //create .csv download link
        $download.style.display="inline";
        copy.unshift(["REGION","CÓDIGO DANE DEL DEPARTAMENTO","DEPARTAMENTO","CÓDIGO DANE DEL MUNICIPIO","MUNICIPIO"]);
    ...
    $search.addEventListener("input",()=>{  //search input
        copy=dataArray.filter(row=>{
            return ((row[0].toLowerCase().includes($search.value.toLowerCase()))||(row[1].toLowerCase().includes($search.value.toLowerCase()))||(row[2].toLowerCase().includes($search.value.toLowerCase()))||(row[3].toLowerCase().includes($search.value.toLowerCase()))||(row[4].toLowerCase().includes($search.value.toLowerCase())));
        });
    ...
    $previus.addEventListener("click",()=>{  //previus button
